...
Total incidents of political violence in Arrondissment Port-au-Prince, in tabular form:

```{r}

table_by_year <- xtabs(~ sub_event_type + event_date_year, data = port_au_prince_civilian_violence, sparse = TRUE)
dimnames(table_by_year)<-setNames(dimnames(table_by_year),c("Sub-Event Type","Year"))
table_by_year
...
CrossTable (port_au_prince_civilian_violence$sub_event_type, port_au_prince_civilian_violence$event_date_year, max.width = 1, prop.chisq = FALSE, prop.r = FALSE, prop.c = TRUE, prop.t = FALSE)
